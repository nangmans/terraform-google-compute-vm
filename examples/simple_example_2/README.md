...
- simple_example Designated Directory에는 아래 작업이 구현되어 있습니다.
  ...
  - Boot Disk(EXISITNG) - Blank Disk Type
    - Boot Disk는 존재하는 디스크를 통해 배포하도록 설정하였으며, Blank Disk로 설정되어 있음
  - Attached Disks는 Zoanl Disk로 설정 되어 있음 
  - External IP는 임시로 할당하여 외부와 통신이 가능하도록 설정 되어 있음
  - Machine Type은 e2-micro로 설정 되어 있음
  - VM Sheild Booting Option이 설정 되어 있음
  - Service Account의 경우, 전에 생성된 Service Account에 대해 Attached 되는것을 확인함
  - Access Scopes은 ["cloud-platform"]을 기준으로 사용

...
- simple_example Designated Directory에는 아래 작업이 구현되어 있습니다.
  ...
  - Instance 옵션
    ...
    - Boot Disk(IMAGE) 
      - Boot Disk는 IMAGE를 통해 배포 되도록 설정 되어 있음
      - OS(CentOS 7)
    - Attached Disk(1)
    ...
    - KMS 관련 공유 내용
      - SNAPSHOT의 KMS Key와 Attached Disks의 Key를 확인하여, Key가 만료가 되었을 경우, 신규 스냅샷을 생성후 추가 디스크를 생성해야함
      - Disk의 KMS Key 버전은 등록하지 않고, 무조건 최신 버전만 바라보도록 설정해야함.
      - KMS 키 버전 업그레이드를 위해선 아래 가이드를 참고해야함[KMS_Key_순환](https://cloud.google.com/compute/docs/disks/customer-managed-encryption#rotate_encryption)
    ...
    - Service Account의 경우, 전에 생성된 Service Account에 대해 Attached 되는것을 확인
